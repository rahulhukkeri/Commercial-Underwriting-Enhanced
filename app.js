...
const app = {
  ...
  getFilteredQuotes() {
    ...
    // 5. Branch Filter (Southern/Central vs Northern/Eastern Thailand coordination)
    const branchEl = document.getElementById('filter-branch');
    if (branchEl && branchEl.value !== 'all') {
      if (branchEl.value === 'London') { // LondonHQ represents Southern/Central (BKK/Rayong/Pattaya)
        list = list.filter(q => q.latitude < 14.5);
      } else {
        list = list.filter(q => q.latitude >= 14.5);
      }
    }
  ...
  changeMapRadius(val) {
    // Rebuild map on radius change
    const mapEl = document.getElementById('leaflet-map');
    if (mapEl && mapEl._leaflet_id && this.leafletMap) {
